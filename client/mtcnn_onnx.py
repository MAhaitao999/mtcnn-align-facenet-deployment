...
class mtcnn(object):
    ...
    def detectFace(self, img, threshold):
        ...
        for scale in scales:
            ...
            t1 = time.time()
            output = self.triton_client.infer("pnet_onnx", inputs=pnet_inputs, outputs=pnet_outputs)
            t2 = time.time()
        ...
        t1 = time.time()
        output = self.triton_client.infer("rnet_onnx", inputs=rnet_inputs, outputs=rnet_outputs)
        t2 = time.time()
        ...
        t1 = time.time()
        output = self.triton_client.infer("onet_onnx", inputs=onet_inputs, outputs=onet_outputs)
        t2 = time.time()
